...
func NewServer(cfg *config.Config, h *Handlers, tokenManager token.Manager, svcs *Services) (*http.Server, *middleware.AuthRateLimiter, *middleware.AuthRateLimiter, *middleware.AuthRateLimiter, *middleware.AuthRateLimiter) {
	...
	router := transportHttp.NewRouter(cfg, h.Auth, h.User, h.PasswordReset, h.Admin, h.Upload, h.Health, tokenManager, svcs.User, svcs.Permission, loginLimiter, registerLimiter, forgotLimiter, resendLimiter)
